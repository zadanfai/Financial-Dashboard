...
date_picker_from = DatePicker(title ="Start Date", value ="2024-01-01", min_date="2000-01-01", 
                                max_date=dt.datetime.now().strftime("%Y-%m-%d"))
date_picker_to = DatePicker(title ="End Date", value ="2024-02-01", min_date="2000-01-01", 
                                max_date=dt.datetime.now().strftime("%Y-%m-%d"))
indicator_chocie = MultiChoice(options=["100 Day SMA", "30 Day SMA", "Linear Regression Line"])
...
load_button.on_click(lambda: on_button_click(stock1_text.value, stock2_text.value,
                                            ...
                                            indicator_chocie.value))

layout = column(stock1_text, stock2_text, date_picker_from, date_picker_to, indicator_chocie, load_button)
